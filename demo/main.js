	...
	window.jsPlumbDemo = {
		init : function() {
			...
			var exampleEndpoint = {
				...
				connectorStyle : {
					gradient : {
						stops : [ [ 0, exampleColor ], [ 0.5, "#09098e" ],
								[ 1, exampleColor ] ]
					},
					lineWidth : 5,
				...
				beforeDrop : function(params) {
					return confirm("Connect " + params.sourceId + " to "
							+ params.targetId + "?");
				},
			...
			//
			// the second example uses a Dot of radius 15 as the endpoint
			// marker, is both a source and target,
			...
			//
			// the third example uses a Dot of radius 17 as the endpoint marker,
			// is both a source and target, and has scope
			// 'exampleConnection3'. it uses a Straight connector, and the
			// Anchor is created here (bottom left corner) and never
			// overriden, so it appears in the same place on every element.
			//
			// this example also demonstrates the beforeDetach interceptor,
			// which allows you to intercept
			// a connection detach and decide whether or not you wish to allow
			// it to proceed.
			//			
